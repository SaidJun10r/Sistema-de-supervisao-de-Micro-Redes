...
for i in range(96):
    if bd['CargaVE'][i] != 0 and horaDescarga == 0: # Verifica se há um carregamento
        horaDescarga = i
        horaCarga = i 
while acumulado <= MaxBateria:
    horaCarga -= 1
    acumulado += acumulado*(bd['Previsao'][horaCarga])/divisao
    divisao += 1
    print(horaCarga, 'Previsão:', bd['Previsao'][horaCarga], 'Acumulado:', acumulado, 'Divisor:', divisao)
...
for i in range(96):
    ...
    if bd['CargaVE'][i]>0: # Horário de desbd['Carga']
        if Bateria[i-1] > 0:
